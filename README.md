...
mkdir logs
```
3. Для создания базы данных создайте папку `instance`. Вот команда в терминале:

```
mkdir instance
```
4. Создайте файл `sqlite.db` в папке `instance`.

```
touch instance/sqlite.db
```
5. Перед запуском тестов настройте изолированную среду, как в разделе 3. Вот команда для запуска тестов.
