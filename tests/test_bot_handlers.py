...
async def test_command_start_handler(mock_router,mock_message):
    ...
    async def test_command_help_handler(mock_router, mock_message):
        ...
        called_args, called_kwargs = mock_message.answer.call_args
        print(called_kwargs)
        assert called_args[0] == "Это помощь по боту!\nСписок команд: /start, /help"

    @pytest.mark.asyncio
    async def test_callback_handler(mock_router, mock_callback_query):
        ...
        assert mock_callback_query.answer.called, "callback_query.answer не был вызван"
